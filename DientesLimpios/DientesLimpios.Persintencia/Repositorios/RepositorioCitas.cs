    ...
    public class RepositorioCitas : Repositorio<Cita>, IRepositorioCitas
    {
        ...
        public async Task<bool> ExisteCitaSolapada(Guid dentistaId, DateTime inicio, DateTime fin)
        {
            return await _context.Citas
                .Where(x => x.DentistaId == dentistaId && x.Estado == EstadoCita.Programada && inicio < x.IntervaloDeTiempo.Fin && fin > x.IntervaloDeTiempo.Inicio)
                .AnyAsync();
        }
        ...
        public async Task<IEnumerable<Cita>> ObtenerFiltrado(FiltroCitasDTO filtroCitasDTO)
        {
            ...
            return await queryable.Where(x => x.IntervaloDeTiempo.Inicio >= filtroCitasDTO.FechaInicio && x.IntervaloDeTiempo.Fin < filtroCitasDTO.FechaFin)
